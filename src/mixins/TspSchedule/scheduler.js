      ...
      if (lenSelectedRows && lenRoles && this.addForm.discipline) {
        ...
        selectedRows.forEach((element) => {
          this.addForm.roles.forEach((el) => {
            ...
            const discipline = this.tspDisciplines.find(
              (element) => element.id === this.addForm.discipline
            );
            ...
            if (this.addForm.shift)
              item.task_name = item.task_name + " - Night";
    ...
    gantt: function () {
      ...
      return {
        ...
        actionBegin: function (args) {
          switch (args.requestType) {
            ...
            case "filterchoicerequest": {
              const gridObj = document.getElementById(self.objectID)
                .ej2_instances[0];
        ...
        menuValidate: function (args) {
          ...
          const selectedRows = ganttObj.selectionModule.getSelectedRecords();
          selectedRows.forEach((element) => {
            if (element.taskData.calloff_status_id === 0) {
              // DELETED
              ganttObj.deleteRecord(element);
            } else if (element.taskData.calloff_status_id === 2) {
              // CHANGED
        ...
        menuResaveCallOff: function (args) {
          const ganttObj = document.getElementById(self.objectID)
            .ej2_instances[0];
          const selectedRows = ganttObj.selectionModule.getSelectedRecords();
          const callOffTasks = ganttObj.dataSource.filter(
            (element) => element.calloff_id === selectedRows[0].calloff_id
          );
        ...
        menuAddTimeOff: function (args) {
          ...
          const selectedRows = ganttObj.selectionModule.getSelectedRecords();
          selectedRows.forEach((element) => {
            const newStartDate = date.addToDate(element.taskData.finish_date, {
              days: 1,
            });
            const timeoff = {
        ...
        menuAddQuarantine: function (args) {
          const ganttObj = document.getElementById(self.objectID)
            .ej2_instances[0];
          const newStartDate = date.subtractFromDate(
            args.rowData.taskData.start_date,
            { days: 15 }
          );
          const quarantine = {
